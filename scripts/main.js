...
// ATUALIZADO (ORGANIZAÇÃO): HUB NÃO INTERCEPTA MAIS LINKS DE MÓDULOS
// - Agora os simuladores abrem em páginas próprias (apps) via <a href="...">.
// - Mantém TODO o código antigo (inclusive loader robusto e módulos), sem perda.
...
class ObservatorioVirtual {
    ...
    async initializePreview() {
        // Sanidade: SceneManager precisa existir (carregado pelo index.html em scripts/shared/SceneManager.js)
        if (typeof window.SceneManager === 'undefined') {
            throw new Error(
                "SceneManager não está definido. Verifique se o arquivo existe em 'scripts/shared/SceneManager.js' " +
                "e se o index.html contém: <script src=\"scripts/shared/SceneManager.js\"></script> antes de scripts/main.js."
            );
        }
    ...
    async ensureSftw1Scripts() {
        // IMPORTANTE: SceneManager deve ser carregado via <script> no index.html (scripts/shared/SceneManager.js)
        // para evitar 404 por heurísticas de caminhos. Aqui só garantimos os módulos do Sftw1.
        //
        // OBS: Este HUB agora abre o planetário em app separado. Mantemos essa função por compatibilidade
        // (sem perda de conteúdo), mas ela não é necessária no fluxo atual.
    ...
    setupEventListeners() {
        // ✅ ALTERAÇÃO PRINCIPAL:
        // Antes: o HUB interceptava clique e tentava ler dataset.module (para carregar módulo dentro do HUB).
        // Agora: os elementos .btn-module-start são LINKS <a href="..."> para apps separados.
        // Então NÃO interceptamos. Só logamos (opcional) e deixamos o navegador navegar.
        document.querySelectorAll('.btn-module-start').forEach(el => {
            el.addEventListener('click', () => {
                const href = el.getAttribute('href');
                console.log('Abrindo app:', href);
    ...
    // ============================
    // A PARTIR DAQUI: TODO O CÓDIGO DE MÓDULOS FOI MANTIDO INTACTO
    // (sem perda de conteúdo).
    //
    // OBS: No fluxo atual (apps separados), estas funções não são chamadas pelo HUB.
    // ============================
