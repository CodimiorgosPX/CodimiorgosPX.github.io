...
class SceneManager {
    ...
    onWindowResize() {
        ...
        // Alguns layouts/CSS podem deixar o canvas com clientHeight = 0 temporariamente
        // (ex.: mudanças no painel de controles). Se redimensionarmos para 0, tudo fica preto.
        // Então: pega dimensões do parent / bounding box e só aplica se for válido.
        const rect = canvas.getBoundingClientRect();
        let w = Math.max(0, rect.width || canvas.clientWidth || 0);
        let h = Math.max(0, rect.height || canvas.clientHeight || 0);
        ...
        // Se ainda não temos tamanho válido, não faz nada (evita setSize(0,0))
        if (h <= 1 || w <= 1) return;
